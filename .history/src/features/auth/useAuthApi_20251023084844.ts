...
export function useSubmitTeachingProfile() {
  return useMutation<
    ...
    any
  >({
    mutationFn: async (teachingData: any) => {
      // For now, simulate API call - replace with actual endpoint
      // TODO: Create FormData for file uploads
      const formData = new FormData();
      
      // Add non-file fields
      Object.keys(teachingData).forEach(key => {
        if (key !== 'video_introduction' && teachingData[key] !== null && teachingData[key] !== undefined) {
          if (Array.isArray(teachingData[key])) {
            formData.append(key, JSON.stringify(teachingData[key]));
          } else {
            formData.append(key, String(teachingData[key]));
          }
        }
      });
