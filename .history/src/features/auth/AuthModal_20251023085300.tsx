...
import { useLogin, useRegister, useSubmitTeachingProfile } from "./useAuthApi";
...
export function AuthModal() {
  ...
  const teachingProfileMutation = useSubmitTeachingProfile();
  ...
  const handleBasicRegister = () => {
    ...
    // Build basic payload - only essential fields for account creation
    const basicPayload = {
    ...
    registerMutation.mutate(basicPayload, {
      onSuccess: () => {
        toast({
          title: "Account created successfully",
          description: formRegister.user_type === "tutor" 
            ? "Please complete your teaching profile." 
  ...
  const handleTeachingProfileSubmit = () => {
    ...
    // Build teaching profile payload
    const teachingPayload = {
      teaching_languages: formRegister.teaching_languages,
      video_introduction: formRegister.video_introduction,
      about_me: formRegister.about_me,
      me_as_teacher: formRegister.me_as_teacher,
      lessons_teaching_style: formRegister.lessons_teaching_style,
      teaching_materials: formRegister.teaching_materials,
      has_webcam: formRegister.has_webcam,
      video_requirements_agreed: formRegister.video_requirements_agreed,
      education_experience: formRegister.education_experience,
      teaching_certificates: formRegister.teaching_certificates,
      teaching_experience: formRegister.teaching_experience,
      industry_experience: formRegister.industry_experience,
      specialty_certificates: formRegister.specialty_certificates,
      profile_visibility: formRegister.profile_visibility,
      teaching_interests: formRegister.teaching_interests,
    };

    // Submit teaching profile to backend
    teachingProfileMutation.mutate(teachingPayload, {
      onSuccess: () => {
        toast({
          title: "Teaching profile completed successfully",
          description: "Please check your email for verification and approval.",
          status: "success",
          position: "top-right",
        });
        openLogin();
      ...
      onError: (err) => {
        toast({
          title: "Teaching profile submission failed",
          description: err.message,
          status: "error",
          position: "top-right",
        });
  ...
  const loading = loginMutation.isPending || registerMutation.isPending || teachingProfileMutation.isPending;
                    ...
                    >
                      ...
                      <Stack direction="row" justify="space-between" mb={4}>
                        ...
                        <Button
                          ...
                          isLoading={loading}
                        >
                          {formRegister.user_type === "tutor" ? "Submit & Continue" : "Create Account"}
                        </Button>
                      </Stack>
                      <Divider />
