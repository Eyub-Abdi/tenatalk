...
export default function Overview() {
  ...
  const earningsSummary = useMemo(() => {
    ...
    const previousWeeks = Math.max(monthLessons - weekLessons, 0);
    const previousWeeklyAverage = previousWeeks > 0 ? previousWeeks / Math.max(Math.floor((monthLessons || 1) / Math.max(weekLessons ? 1 : 4, 1)), 1) : 0;
    const changeBase = previousWeeklyAverage || (monthLessons ? monthLessons / 4 : 0);
