...
export default function CreateLesson() {
  ...
  const durationMinutes = useMemo(() => (duration === "30" ? 30 : 60), [duration]);
  const priceLabel = useMemo(() => new Intl.NumberFormat(undefined, {
    style: "currency",
    currency: "USD",
  ...
  const handleSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : "Unable to create lesson.";
        toast({ title: "Something went wrong", description: message, status: "error" });
      }
    },
    [
      createLesson,
      ...
      <Stack spacing={6}>
        <Stack spacing={2}>
          ...
          <Text fontSize="sm" color={mutedText}>
            Craft lesson titles like on italki—be specific about outcomes so students pick the right fit.
          </Text>
        </Stack>

        <Stack spacing={6} bg={cardBg} borderRadius="md" border="1px solid" borderColor={borderColor} p={{ base: 4, md: 6 }} shadow="sm">
          ...
          <Stack spacing={3}>
            ...
            <RadioGroup value={duration} onChange={(value) => setDuration(value as "30" | "60") }>
                  ...
                  return (
                    ...
                    >
                      <HStack justify="space-between" align="start">
                        ...
                        {isActive && <Badge colorScheme="brand">Selected</Badge>}
                      </HStack>
          ...
          <Stack spacing={3}>
            ...
            <RadioGroup value={format} onChange={(value) => setFormat(value as "single" | "package")}>
              <SimpleGrid columns={{ base: 1, md: 2 }} spacing={3}>
                {formatOptions.map((option) => {
                  ...
                  return (
                    <Box
                      ...
                      cursor="pointer"
                      bg={isActive ? "brand.50" : "transparent"}
                      onClick={() => setFormat(option.value as "single" | "package")}
                      transition="all 0.2s"
                    >
                      <HStack justify="space-between" align="start">
                        <Radio value={option.value}>{option.label}</Radio>
          ...
          <Stack spacing={2}>
            ...
            <Text fontSize="sm" color={mutedText}>
              {durationMinutes} minutes · {priceLabel} · {format === "single" ? "single lesson" : "package lesson"}
            </Text>
          </Stack>
