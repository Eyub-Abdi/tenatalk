...
async function createLesson(payload: LessonCreatePayload) {
  return http<LessonDetail>("/lessons/", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
...
export function useCreateLesson() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (payload: LessonCreatePayload) => createLesson(payload),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: lessonKeys.lists() });
      qc.invalidateQueries({ queryKey: lessonKeys.stats() });
    },
  });
}
